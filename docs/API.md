...
- **Обновления** (`plugins.updater.endpoints` в `tauri.conf.json`) — идут
  не на `API_BASE`, а на отдельный прокси `/api/desktop/update` и
  `/api/desktop/update/asset/{id}` того же сервера — см. `CHANGELOG.md`
  (v0.4.1) про то, зачем нужен прокси для приватного GitHub-репозитория
